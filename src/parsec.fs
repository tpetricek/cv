...
let anySpace = zeroOrMore (pred (fun t -> System.Char.IsWhiteSpace t))
...
let string = oneOrMore letter |> map (fun ch -> new System.String(Array.ofList ch))

let stringWithNestedBrackets = Parser(fun (i, c) ->
  let rec loop acc brackets i c = 
    match c with 
    | [] -> None
    | '{'::c -> loop acc (brackets+1) (i+1) c
    | '}'::c when brackets = 0 -> Some((i, '}'::c), System.String(Array.ofList(List.rev acc)) )
    | '}'::c -> loop acc (brackets-1) (i+1) c
    | c::cs -> loop (c::acc) brackets (i+1) cs
  loop [] 0 i c )
